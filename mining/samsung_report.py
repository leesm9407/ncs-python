...
class SamsungReport(object):
    ...
    def extract_hangeul(texts):
        temp = texts.replace('\n', ' ')
        tokenizer = re.compile(r'[^ ㄱ-힣]+')    # r: 정규표현식, 한글이 아닌 것까지
        temp = tokenizer.sub('', temp)          # 한글 제외하고 ' ' 처리
        return temp
    ...
    def hook(self):
        ...
        freqtxt = pd.Series(dict(FreqDist(texts))).sort_values(ascending=False)     # FreqDist : 반복될때마다 count 올림
        print(freqtxt[:30])
        return freqtxt
